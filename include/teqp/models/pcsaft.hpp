...
public:
    ...
    auto alphar(const TTYPE& T, const RhoType& rhomolar, const VecType& mole_fractions) const {
        ...
        c.d = sigma_Angstrom.cast<TTYPE>()*(1.0 - 0.12*exp(-3.0*epsilon_over_k.cast<TTYPE>()/T)); // [A]
        ...
        for (std::size_t n = 0; n < 4; ++n) {
            ...
            TRHOType xmdn = forceeval((mole_fractions.cast<TRHOType>()*m.cast<TRHOType>()*dn).sum());
