...
/**
 Take derivatives of properties w.r.t. T and rho
 */
template <typename Model, typename Ideal, typename Scalar, typename VecType>
auto build_iteration_Jv(const std::vector<char>& vars, Model& resid, const Ideal& ideal, const Scalar& T, const Scalar& rho, const VecType &z){
    IterationMatrices im; im.J.resize(vars.size(), 2); im.v.resize(vars.size()); im.vars = vars;
    auto Ar = DerivativeHolderSquare<2, AlphaWrapperOption::residual>(resid, T, rho, z).derivs;
    auto Aig = DerivativeHolderSquare<2, AlphaWrapperOption::idealgas>(ideal, T, rho, z).derivs;
    auto A = Ar + Aig;
    auto R = resid.R(z); // Have to worry about the differing R values (sigh...)
